...
public static class SweepHandler {
    public const string TypeName = "Sweep";

    // SW quirk: GetTypeName2() returns "SweepFeature" for boss sweeps.
    public const string SwTypeName = "SweepFeature";

    // SW quirk: InsertProtrusionSwept4 reads selections from these marks.
    ...
    // SW quirk: -1 = None (SW collapses to Both at create time); 0/1/2 = DirectionA/Both/DirectionB.
    ...
    private static short ResolveTangency(string s) => s switch {
        ...
        _ => throw new ArgumentException($"Sweep: unknown tangency literal '{s}'"),
    };
    ...
    public static JsonNode Add(File file, JsonNode input) {
        ...
        if (args.ProfileType == WireProfileType.Solid) {
            throw new NotSupportedException(
                "Sweep: profile_type='Solid' is forward-compat schema only; "
                + "handler implementation deferred until a real authoring case lands");
        }

        // SW quirk: InsertProtrusionSwept4 silently returns null on a class of valid inputs that
        // the data-driven CreateDefinition + CreateFeature path handles fine.
        ...
        var data = fm.CreateDefinition((int)swFeatureNameID_e.swFmSweep) as SweepFeatureData
            ?? throw new InvalidOperationException(
                "Sweep: CreateDefinition(swFmSweep) did not return a SweepFeatureData");
        ...
        if (feature is null) {
            throw new InvalidOperationException(
                "Sweep: CreateFeature failed — check profile is closed, path is connected, and (if guide curves) they're tangent to the profile");
        }

        SldworksLog.Information("SweepHandler.Add: created {Name}", feature.Name);
    ...
    private static void PopulateSweepData(SweepFeatureData data, SweepArgs args) {
        var t = args.Twist;
        // Circular profile uses SW's built-in "create circle around path" instead of a sketch.
        if (args.ProfileType == WireProfileType.Circular) {
            data.CircularProfile = true;
            data.CircularProfileDiameter = args.CircularDiameter
                ?? throw new ArgumentException("Sweep: profile_type='Circular' requires circular_diameter");
        } else {
        ...
        // Direction=None=-1 is the "unspecified" sentinel; writing -1 directly to FeatureData
        // makes CreateFeature reject otherwise-valid input on some configurations.
    ...
    // Edit an existing boss sweep IN PLACE (GetDefinition -> AccessSelections -> mutate ->
    // ModifyDefinition) so the feature name — and every downstream name-ref / probe —
    // survives; we never delete + re-add. `input` is a FULL sweep payload (same shape
    // Inspect emits / Add consumes), parsed by SweepArgs so edit and create share one
    // validation. Parametric scalars/options/flags are applied — exactly the fields
    // Inspect READS — AND the re-pointable geometry selections (Profile sketch, Path,
    // GuideCurves) are re-resolved + re-set unconditionally from the provided refs
    // (re-applying the same ref is idempotent; ModifyDefinition's bool return is the
    // validity gate — no compare-and-skip). What
    // stays as-built: FeatureScopeBodies (Inspect reads it but it's an AutoSelect-driven
    // scope, not an authored profile/path/guide ref) and the twist=Direction vector
    // (a mark-128 selection refused upstream).
    //
    ...
    public static JsonNode Edit(File file, Feature feature, JsonNode input) {
        ...
        if (args.ProfileType == WireProfileType.Solid) {
            throw new NotSupportedException(
                "Sweep.Edit: profile_type='Solid' is forward-compat schema only; "
                + "handler implementation deferred until a real authoring case lands");
        }

        var data = feature.GetDefinition() as ISweepFeatureData
            ?? throw new InvalidOperationException(
                $"Sweep.Edit: feature {feature.Name} does not expose ISweepFeatureData");
        ...
        if (currentlyCircular != requestedCircular) {
            throw new InvalidOperationException(
                $"Sweep.Edit: profile_type can't be changed in place on '{feature.Name}' "
        ...
        if (args.Twist.Kind == WireTwistKind.Direction) {
            throw new InvalidOperationException(
                $"Sweep.Edit: twist='Direction' can't be changed in place on '{feature.Name}' "
                + "— its direction reference is a geometry selection; delete and re-add");
        }

        if (!data.AccessSelections(file.ModelDoc, null)) {
            throw new InvalidOperationException(
                $"Sweep.Edit({feature.Name}): AccessSelections failed");
        }
        try {
            ...
            if (!feature.ModifyDefinition(data, file.ModelDoc, null)) {
                throw new InvalidOperationException(
                    $"Sweep.Edit: ModifyDefinition returned false on '{feature.Name}' "
        ...
        // AfterFeature: a mid-tree sweep must not roll later features back.
        file.MoveRollbackBar(swMoveRollbackBarTo_e.swMoveRollbackBarToEnd);
        SldworksLog.Information("SweepHandler.Edit: {Name}", feature.Name);
    ...
    private static void ApplyEditableFields(ISweepFeatureData data, SweepArgs args) {
        ...
        if (args.ProfileType == WireProfileType.Circular) {
            data.CircularProfileDiameter = args.CircularDiameter
                ?? throw new ArgumentException("Sweep.Edit: profile_type='Circular' requires circular_diameter");
        }
        ...
        switch (t.Kind) {
            ...
            case WireTwistKind.Direction:
                ...
                throw new InvalidOperationException(
                    "Sweep.Edit: twist='Direction' is not in-place editable");
        }
    ...
    private static void RepointProfileIfChanged(
        ...
        if (string.IsNullOrEmpty(requestedName)) {
            throw new InvalidOperationException(
                $"Sweep.Edit: profile sketch name missing on '{featureName}' — a Sketch sweep "
        ...
        var newFeat = FindFeatureByName(file, resolvedRequested)
            ?? throw new InvalidOperationException(
                $"Sweep.Edit: profile sketch '{requestedName}' not found on '{featureName}'.");
        data.Profile = newFeat;
        SldworksLog.Information(
            "SweepHandler.Edit: re-pointed profile of '{Name}' onto sketch '{Sketch}'",
    ...
    private static void RepointPathIfChanged(
        ...
        if (live is null) {
            throw new InvalidOperationException(
                $"Sweep.Edit: path ({pathDef.GetType().Name}) did not resolve to a live entity "
        ...
        data.Path = live;
        SldworksLog.Information(
            "SweepHandler.Edit: re-pointed path of '{Name}' onto {LiveType}",
    ...
    private static void RepointGuideCurvesIfChanged(
        ...
        for (var i = 0; i < guideDefs.Count; i++) {
            ...
            if (live is null) {
                throw new InvalidOperationException(
                    $"Sweep.Edit: guide_curves[{i}] ({guideDefs[i].GetType().Name}) did not resolve "
        ...
        SldworksLog.Information(
            "SweepHandler.Edit: re-pointed guide curves of '{Name}' onto {Count} curve(s)",
    ...
    public static JsonNode Inspect(File file, Feature feature) {
        ...
        if (!data.AccessSelections(file.ModelDoc, null)) {
            throw new InvalidOperationException(
                $"Sweep.Inspect({feature.Name}): AccessSelections failed");
        }
        try {
            // Circular profile (data.CircularProfile) uses a built-in circle around the path
            // instead of a sketch profile — Profile is null in that case.
            ...
            // SW quirk: pre-SW-2010 features have Direction=None which SW treats as Both; surface the raw enum.
            ...
            // SW quirk: ReadTwist → ProbeSpecifiedD1Sign may call ModifyDefinition,
            // which releases AccessSelections under `data`. Subsequent reads off `data`
            // (FeatureScopeBodies in particular) silently return empty when access is
            // released. Re-fetch the live definition and re-acquire before reading the
            // remaining fields.
            if (twistJson["twist"]?.GetValue<string>() == "Specified") {
                data = (SweepFeatureData)feature.GetDefinition();
                if (!data.AccessSelections(file.ModelDoc, null)) {
                    throw new InvalidOperationException(
                        $"Sweep.Inspect({feature.Name}): AccessSelections failed after twist sign probe");
                }
            }
            ...
            // SW quirk: GetWallThickness(false) reads direction 1 (outward); direction 2 is only meaningful for two-direction thin walls.
            if (data.ThinFeature) {
    ...
    private static JsonNode CapturePath(File file, object? path) {
        ...
        if (path is null) {
            throw new InvalidOperationException("Inspect(Sweep): path is null");
        }
        var def = DefinitionCapture.Capture(path)
            ?? throw new InvalidOperationException(
                $"Inspect(Sweep): path is unsupported runtime type {path.GetType().Name}");
        // SW quirk: a sketch reused by multiple features comes back through the
        // consumer with a "<N>" suffix indicating Nth-consumer. FeatureName.Resolve
        // strips it back to the canonical top-level name so the round-trip diff
        // doesn't trip when source and target have different consumer counts.
    ...
    private static JsonObject ReadTwist(File file, Feature feature, SweepFeatureData data) {
        ...
        switch (tct) {
            ...
            case swTwistControlType_e.swTwistControlNormalConstantTwistAlongPath:
                ...
                // SW quirk: `D1ReverseTwistDir`'s getter on `SweepFeatureData`
                // returns False regardless of the authored UI direction. Recover
                // the sign via the shared rollback + force-clear + body-diff probe.
                var d1Sign = TwistDirSignProbe.ProbeD1(file, feature, data);
                var angleA = Math.Abs(data.GetTwistAngle()) * d1Sign;
                double? angleB = null;
                var rawB = data.GetD2TwistAngle();
                if (data.D2ReverseTwistDir) rawB = -Math.Abs(rawB);
                if (Math.Abs(rawB) > 1e-12) angleB = rawB;
    ...
    private static JsonArray CollectBodies(File file) {
        ...
        // SW quirk: arg 2 is `bVisibleOnly` — passing `true` drops hidden
        // bodies, which silently undercounts when an upstream sweep merged
        // into a body that SW left hidden. Pass `false` to capture every
        // solid body in the doc, matching FilletHandler's convention.
    ...
    private static void SelectProfile(File file, string sketchName) {
        if (string.IsNullOrEmpty(sketchName)) {
            throw new ArgumentException("Sweep: profile sketch name missing — sketch must be added first");
        }
        var feat = FindFeatureByName(file, sketchName)
            ?? throw new ArgumentException($"Sweep profile sketch '{sketchName}' not found");
        feat.Select2(true, MarkProfile);
    }

    private static void SelectPath(File file, Definition path) {
        if (!path.Select(file, MarkPath)) {
            throw new InvalidOperationException(
                $"Sweep: path Definition ({path.GetType().Name}) did not resolve+select onto mark {MarkPath}");
        }
    }

    private static void SelectGuideCurves(File file, IReadOnlyList<Definition> guideCurves) {
        Definition.SelectAll(file, guideCurves, MarkGuideCurves, "Sweep.guide_curves");
    }
    ...
    private static void SelectTwistDirection(File file, WireTwistPayload t) {
        ...
        if (!t.Direction.Select(file, MarkTwistDirection)) {
            throw new InvalidOperationException(
                $"Sweep: twist-direction Definition ({t.Direction.GetType().Name}) did not resolve+select onto mark {MarkTwistDirection}");
        }
    }
    ...
    private static void SelectFeatureScopeBodies(File file, IReadOnlyList<Definition>? bodies) {
        ...
        Definition.SelectAll(file, bodies, MarkFeatureScopeBodies, "Sweep.feature_scope");
    }
    ...
    private sealed record SweepArgs(
        ...
        internal static SweepArgs Parse(JsonNode input) {
            ...
            var pathNode = input["path"]
                ?? throw new ArgumentException("Sweep: missing 'path'");
            var path = Definition.FromJson(pathNode)
                ?? throw new ArgumentException("Sweep: 'path' could not be parsed as a Definition");
            ...
            if (input["guide_curves"] is JsonArray gcArr) {
                foreach (var node in gcArr) {
                    ...
                    var d = Definition.FromJson(node)
                        ?? throw new ArgumentException("Sweep: guide_curves entry is not a Definition");
                    guideCurves.Add(d);
                }
            }
            ...
            var profileType = profileTypeStr switch {
                ...
                _ => throw new ArgumentException($"Sweep: unknown profile_type '{profileTypeStr}'"),
            };
            ...
            if (input["feature_scope"] is JsonArray fsArr) {
                ...
                foreach (var node in fsArr) {
                    if (node is null) continue;
                    var d = Definition.FromJson(node)
                        ?? throw new ArgumentException("Sweep: feature_scope entry is not a Definition");
                    list.Add(d);
                }
                // Empty array collapses to null so AutoSelect takes over.
                featureScope = list.Count > 0 ? list : null;
            }

            var direction = ParseSweepDirection(input["direction"]?.GetValue<string>());

            return new SweepArgs(profileName, path, guideCurves, twist, align, mergeTangent, merge,
    ...
    private static WireSweepDirection ParseSweepDirection(string? s) => s switch {
        ...
        _ => throw new ArgumentException($"Sweep: unknown direction literal '{s}'"),
    };
        ...
        internal static WireTwistPayload? Parse(JsonNode? node) {
            ...
            return name switch {
                ...
                "Direction"            => new(WireTwistKind.Direction,           0.0,
                                              ...
                                              ?? throw new ArgumentException("Sweep twist=Direction requires a 'direction' Definition"),
                                              null, false),
