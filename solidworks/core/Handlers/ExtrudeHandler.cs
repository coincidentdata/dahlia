...
public static class ExtrudeHandler {
    public const string TypeName = "Extrude";

    // SW quirk: GetTypeName2() returns "Extrusion" for boss extrudes.
    public const string SwTypeName = "Extrusion";

    // ---- Add -----------------------------------------------------------------------

    public static JsonNode Add(File file, JsonNode input) {
        var args = ExtrudeArgs.Parse(input);
        ...
        // SW quirk: end-condition ref mark=1, contour mark=0, start-entity mark=32, feature-scope mark=8.
        // SW quirk: an empty (but non-null) feature_scope must collapse to AutoSelect —
        // passing UseAutoSelect=false without selecting bodies makes FeatureExtrusion3 fail.
        ...
        // SW quirk: when both end-cond (mark 1) and feature_scope (mark 8) carry Body2,
        // the order they're appended to the selection list determines which body SW picks
        // for UpToBody (feature_scope-first works, end-cond-first substitutes the wrong body).
        ...
        // SW quirk: PromptBodiesToKeepNotify fires for boss extrudes when merge=false on multi-body parts.
        using var bodiesScope = !hasExplicitBodies ? BodiesToKeepScope.Register(file) : null;
        ...
        var feature = (Feature?)fm.FeatureExtrusion3(
        ...
        if (feature is null) {
            // SW quirk: FeatureExtrusion3 returns null without disambiguation on bad sketch/depth/end-condition.
            throw new InvalidOperationException(
                "FeatureExtrusion3 failed; check sketch has closed regions, depth > 0, and end-condition is valid");
        }

        // SW quirk: FeatureExtrusion3 has no slot for direction-2 axis; apply post-create via SetDirectionReference.
        if (args.DirectionB is not null) {
            ApplyDirectionB(file, feature, args.Direction, args.DirectionB);
        }

        SldworksLog.Information("ExtrudeHandler.Add: created {Name}", feature.Name);
    ...
    private static void ApplyDirectionB(File file, Feature feature, Definition? dir1, Definition dir2) {
        var data = feature.GetDefinition() as IExtrudeFeatureData2
            ?? throw new InvalidOperationException(
                "Extrude: could not edit feature data to apply direction_b");
        if (!data.AccessSelections(file.ModelDoc, null)) {
            throw new InvalidOperationException(
                "Extrude: AccessSelections failed when applying direction_b");
        }
        try {
            object? live1 = null;
            if (dir1 is not null) {
                live1 = DefinitionResolver.Resolve(file, dir1)
                    ?? throw new InvalidOperationException(
                        "Extrude: direction (slot 1) did not resolve to a live entity");
            }
            var live2 = DefinitionResolver.Resolve(file, dir2)
                ?? throw new InvalidOperationException(
                    "Extrude: direction_b (slot 2) did not resolve to a live entity");
            data.SetDirectionReference(live1!, live2);
            if (!feature.ModifyDefinition(data, file.ModelDoc, null)) {
                throw new InvalidOperationException(
                    "Extrude: ModifyDefinition failed when applying direction_b");
            }
        } finally {
    ...
    // Edit an existing boss extrude IN PLACE (GetDefinition -> AccessSelections -> mutate
    ...
    // `input` is a FULL extrude payload (same shape Inspect emits / Add consumes), parsed
    // by ExtrudeArgs so edit and create share one validation. Edit applies EVERY field Add
    // does, including the geometry REFERENCES: unlike Add (which selects geometry onto
    // marks and lets FeatureExtrusion3 consume them), Edit resolves each incoming reference
    // Definition via DefinitionResolver and sets it through the I-interface setter inside
    // the AccessSelections block, only when the payload supplies one. Mirrors the Add-side
    // ApplyDirectionB pattern (resolve -> setter -> ModifyDefinition). References handled:
    ...
    public static JsonNode Edit(File file, Feature feature, JsonNode input) {
        ...
        var data = feature.GetDefinition() as IExtrudeFeatureData2
            ?? throw new InvalidOperationException(
                $"Extrude.Edit: feature {feature.Name} ({feature.GetTypeName2()}) does not expose IExtrudeFeatureData2");

        data.AccessSelections(file.ModelDoc, null);
        try {
            ApplyDirection(file, data, primary: true, args.End1, args.DraftAngle, "Extrude");
            data.BothDirections = args.BothDirections;
            if (args.BothDirections) {
                var end2 = args.End2
                    ?? throw new InvalidOperationException(
                        "Extrude.Edit: both_directions is set but end_condition_b is missing");
                ApplyDirection(file, data, primary: false, end2, args.DraftAngleB ?? args.DraftAngle, "Extrude");
            }
            data.ReverseDirection = args.Reversed;
            ApplyStartConditionEdit(file, data, args.Start, "Extrude");
            ApplyDirectionReferenceEdit(file, data, args.Direction, args.DirectionB);
            ApplyContoursEdit(file, data, args.Contours, args.SketchName, "Extrude");
            ...
            if (!feature.ModifyDefinition(data, file.ModelDoc, null)) {
                throw new InvalidOperationException(
                    $"Extrude.Edit: ModifyDefinition returned false on '{feature.Name}' — "
        ...
        // AfterFeature: a mid-tree extrude must not roll later features back.
        file.MoveRollbackBar(swMoveRollbackBarTo_e.swMoveRollbackBarToEnd);
        SldworksLog.Information("ExtrudeHandler.Edit: {Name}", feature.Name);
    ...
    private static void ApplyDirectionReferenceEdit(
        ...
        object? live1 = dir1 is not null
            ? DefinitionResolver.Resolve(file, dir1)
                ?? throw new InvalidOperationException(
                    $"Extrude.Edit: direction (slot 1) ({dir1.GetType().Name}) did not resolve to a live entity")
            : null;
        object? live2 = dir2 is not null
            ? DefinitionResolver.Resolve(file, dir2)
                ?? throw new InvalidOperationException(
                    $"Extrude.Edit: direction_b (slot 2) ({dir2.GetType().Name}) did not resolve to a live entity")
            : null;
    ...
    // the as-built profile alone — Inspect emits an empty array for a whole-sketch extrude).
    private static void ApplyContoursEdit(
        ...
        var ownerSketch = SelectSketchByFeatureFind(file, sketchName)?.GetSpecificFeature2() as Sketch;
    ...
    public static JsonNode Inspect(File file, Feature feature) {
        ...
        // SW quirk: AccessSelections required before reading refs; skipping the release wedges the document.
        data.AccessSelections(file.ModelDoc, null);
        try {
            ...
            var end1 = ExtrudeEndCondition.CaptureEndCondition(data, primary: true, "Extrude");
            JsonNode? end2 = bothDirections
                ? ExtrudeEndCondition.CaptureEndCondition(data, primary: false, "Extrude")
                : null;
    ...
    private static (JsonNode? dir1, JsonNode? dir2) CaptureDirections(IExtrudeFeatureData2 data) {
        // SW quirk: GetDirectionReference exposes two slots (one axis per direction).
        ...
        var d1 = dirRef is null ? null : DefinitionCapture.Capture(dirRef)?.ToJson()
            ?? throw new InvalidOperationException(
                $"Inspect(Extrude): direction reference is unsupported runtime type {dirRef.GetType().Name}");
        var d2 = dirRef2 is null ? null : DefinitionCapture.Capture(dirRef2)?.ToJson()
            ?? throw new InvalidOperationException(
                $"Inspect(Extrude): direction reference is unsupported runtime type {dirRef2.GetType().Name}");
        return (d1, d2);
    }

    // ---- contour capture & selection -----------------------------------------------
    ...
    private static JsonArray CaptureContours(IExtrudeFeatureData2 data) {
        ...
        foreach (var contour in contours) {
            if (contour is null) {
                throw new InvalidOperationException("Inspect(Extrude): contour entry is null");
            }
            arr.Add(DefinitionCapture.Capture(contour)?.ToJson()
                ?? throw new InvalidOperationException(
                    $"Inspect(Extrude): contour entry of unsupported type {contour.GetType().Name} " +
    ...
    private static JsonNode CaptureStartCondition(IExtrudeFeatureData2 data) {
        ...
        switch (fromType) {
            ...
            default:
                throw new InvalidOperationException(
                    $"Inspect(Extrude): unknown swStartCondition_e value {fromType}");
        }
    }
    ...
    private static JsonNode CaptureStartReference(IExtrudeFeatureData2 data, string label) {
        ...
        if (entity is null) {
            throw new InvalidOperationException(
                $"Inspect(Extrude): start.{label} reference is null");
        }
        return DefinitionCapture.Capture(entity)?.ToJson()
            ?? throw new InvalidOperationException(
                $"Inspect(Extrude): start.{label} reference is unsupported runtime type {entity.GetType().Name}");
    }
    ...
    private static void SelectFeatureScopeBodiesForAdd(File file, IReadOnlyList<Definition> bodies) {
        ...
        Definition.SelectAll(file, bodies, MarkFeatureScopeBodies, "Extrude.feature_scope");
    }
    ...
    private static void SelectSketchByFeature(File file, string sketchName) {
        if (string.IsNullOrEmpty(sketchName)) {
            throw new ArgumentException("Extrude: sketch name missing — sketch must be added first");
        }
    ...
    private static Feature? SelectSketchByFeatureFind(File file, string sketchName) {
        if (string.IsNullOrEmpty(sketchName)) return null;
        return FindFeatureByName(file, sketchName);
    }
    ...
    private static void SelectContours(File file, IReadOnlyList<Definition> contours, string sketchName) {
        ...
        // SW quirk: sketch segment IDs reissue per-sketch from low numbers, so restrict the
        // resolver to the owning sketch to avoid matching a same-id segment in another sketch.
        var sketchFeature = SelectSketchByFeatureFind(file, sketchName);
        var ownerSketch = sketchFeature?.GetSpecificFeature2() as Sketch;
        ...
        if (liveEntities.Count == 0) {
            throw new InvalidOperationException(
                $"Extrude: no live contour matched the captured " +
                $"{contours.Count} source contour(s) under segment-/edge-subset rule");
        }
        foreach (var live in liveEntities) {
            switch (live) {
                ...
                case SketchSegment seg:
                    // SW quirk: SketchSegment uses Select4 (not Select2).
                    seg.Select4(true, sd);
                    break;
                default:
                    throw new InvalidOperationException(
                        $"Extrude: contour resolved to unsupported type {live.GetType().Name} " +
    ...
    private static void SelectDirection(File file, Definition directionDef) {
        // SW quirk: direction ref uses default selection (no mark); Append=true preserves mark 0 sketch.
        var live = DefinitionResolver.Resolve(file, directionDef)
            ?? throw new InvalidOperationException("Extrude direction did not resolve to a live entity");
        ...
        switch (live) {
            ...
            default:
                throw new InvalidOperationException(
                    $"Extrude direction resolved to unsupported type {live.GetType().Name}");
        }
    }
        ...
        switch (start.Kind) {
            ...
            case ExtrudeStartKind.Surface:
                if (!start.Reference!.Select(file, startMark)) {
                    throw new InvalidOperationException(
                        $"Extrude: start.Surface Definition ({start.Reference.GetType().Name}) did not resolve+select onto mark {startMark}");
                }
                return ((int)swStartConditions_e.swStartSurface, 0.0, false);
            case ExtrudeStartKind.Vertex:
                if (!start.Reference!.Select(file, startMark)) {
                    throw new InvalidOperationException(
                        $"Extrude: start.Vertex Definition ({start.Reference.GetType().Name}) did not resolve+select onto mark {startMark}");
                }
            ...
            default:
                throw new InvalidOperationException($"Extrude: unknown start kind {start.Kind}");
        }
    }
    ...
    private sealed record ExtrudeArgs(
        string SketchName,
        double Depth,
        bool Reversed,
        bool BothDirections,
        ...
        internal static ExtrudeArgs Parse(JsonNode input) {
            ...
            // Fall back to top-level depth as Blind when end_condition is absent.
            ...
            return new ExtrudeArgs(sketchName, depth, reversed, bothDir, merge,
                draft, draftB, direction, directionB, contours, start, end1, end2, featureScope);
        }
    }

    private static IReadOnlyList<Definition>? ReadOptionalDefinitionList(JsonNode node, string field) {
        // null/absent = AutoSelect; empty array = user-narrowed with zero bodies (must not collapse to null).
        var v = node[field];
