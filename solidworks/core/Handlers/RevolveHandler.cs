...
public static class RevolveHandler {
    public const string TypeName = "Revolve";

    // SW quirk: GetTypeName2() returns "Revolution" for boss revolves.
    public const string SwTypeName = "Revolution";

    // ---- Add -----------------------------------------------------------------------

    public static JsonNode Add(File file, JsonNode input) {
        var args = RevolveArgs.Parse(input);
        // Sequence selections in strictly decreasing mark order: axis (4) → feature_scope (1) → contours/sketch (0).
        // SW quirk: axis must be selected BEFORE contours/sketch so FeatureRevolve2's
        // mark-4 lookup picks it up reliably.
        // Clear once up front — neither SelectContours nor SelectSketch clears below, so
        // higher-mark selections survive.
        ...
        if (args.Contours.Count > 0) {
            ...
            AppendSketch(file, args.SketchName);
        }

        // SW quirk: FeatureRevolve2's Dir1Type / Dir2Type are swEndConditions_e (Blind /
        // MidPlane), NOT swRevolveType_e. The revolve TYPE (one-direction vs mid-plane vs
        // two-direction vs full-360) is implied by SingleDir + the angles; Dir1Type is
        // hardcoded to Blind here. SW classifies a 2π single-direction revolve as
        // swRevolveTypeOneDirection360Degrees on Inspect.
        ...
        if (feature is null) {
            throw new InvalidOperationException(
                "FeatureRevolve2 failed; check sketch has a closed profile, axis is valid, and angle is in (0, 2π].");
        }

        SldworksLog.Information("RevolveHandler.Add: created {Name}", feature.Name);
    ...
    // Edit an existing boss revolve IN PLACE (GetDefinition -> AccessSelections -> mutate ->
    // ModifyDefinition); we do NOT delete + re-add, so the feature name — and every
    // downstream name-ref / probe — survives. `input` is a FULL revolve payload (same shape
    // Inspect emits / Add consumes), parsed by RevolveArgs so edit and create share one
    // validation. Parametric scalars/settings are applied (angle, angle_b, mid_plane,
    // reversed, merge) AND the two re-pointable geometry refs Add resolves+selects — the
    // revolve AXIS (data.Axis) and the profile CONTOURS (data.Contours) — are re-pointed when
    // the payload's refs DIFFER from what's currently built (multiset of canonical Definition
    // JSON). Unchanged refs are left untouched (the common scalar-only edit never disturbs the
    // selection). The owning SKETCH (first sub-feature) is structural and is NOT re-pointed:
    // swapping the parent sketch is a different feature (delete + re-add). Mirrors the Chamfer
    // Edit shape (resolve -> capture -> multiset-compare -> reflection-confirmed setter).
    public static JsonNode Edit(File file, Feature feature, JsonNode input) {
        ...
        var data = feature.GetDefinition() as IRevolveFeatureData2
            ?? throw new InvalidOperationException(
                $"Revolve.Edit: feature {feature.Name} ({feature.GetTypeName2()}) does not expose IRevolveFeatureData2");
        ...
        try {
            ...
            // (re-point before per-side distance/flip).
            ...
            // Boss revolve only: merge is on the wire. (CutRevolve holds Merge=true.)
            data.Merge = args.Merge;
            ...
            if (!feature.ModifyDefinition(data, file.ModelDoc, null)) {
                throw new InvalidOperationException(
                    $"Revolve.Edit: ModifyDefinition returned false on '{feature.Name}' — "
        ...
        // AfterFeature: a mid-tree revolve must not roll later features back.
        file.MoveRollbackBar(swMoveRollbackBarTo_e.swMoveRollbackBarToEnd);
        SldworksLog.Information("RevolveHandler.Edit: {Name}", feature.Name);
    ...
    // Re-point the revolve AXIS in place when the payload's axis DIFFERS from what's currently
    // built. The incoming axis def arrives in the Edit payload (RevolveArgs.Axis, the SAME ref
    // Add resolves+selects via SelectAxis). Here — unlike Add (select-then-create) — we resolve
    // it and assign the data-interface's Axis property directly, inside the open
    // AccessSelections block, before ModifyDefinition.
    //
    // Reflection-confirmed setter: IRevolveFeatureData2.set_Axis(Object) marshals the value as
    // UnmanagedType.IDispatch — a SINGLE live dispatch object, NOT a SAFEARRAY (same shape as
    // Chamfer.IVertex). So we assign the resolved live entity DIRECTLY, with NO DispatchWrapper.
    private static void RepointAxisIfChanged(
        ...
        var live = DefinitionResolver.Resolve(file, axisDef)
            ?? throw new InvalidOperationException(
                $"Revolve.Edit: axis ({axisDef.GetType().Name}) did not resolve to a live entity "
                + $"on '{featureName}'.");

        // Assume the payload is authoritative — just apply the axis (no compare/skip). The payload
        // already carries the current axis for a scalar-only edit (re-applying it is a no-op) or
        // the new axis for a re-point. set_Axis is a scalar IDispatch property — assign the single
        // live entity directly, no DispatchWrapper (contrast data.Contours, a SAFEARRAY).
        data.Axis = live;
    }

    // Re-point the profile CONTOURS in place. The incoming contour defs arrive in the Edit payload
    // (RevolveArgs.Contours, the SAME refs Add resolves+selects via AppendContours). Resolved
    // through the SAME ResolveContoursForSelection Add uses (subset-matched against the owning
    // sketch — contour decomposition isn't stable across rebuilds), then set unconditionally
    // (assume the payload is authoritative; re-applying the same selection is idempotent).
    //
    // Reflection-confirmed setter: IRevolveFeatureData2.set_Contours(Object) marshals as a
    // SAFEARRAY (UnmanagedType.Struct — same shape as Chamfer.Edges / Fillet.Edges), so it wants
    // DispatchWrapper[]; a bare object[] crashes the marshaller.
    //
    // When the payload carries NO explicit contours (Add would have selected the whole sketch),
    // there is nothing an authorable contour-ref edit can re-point — the profile is the sketch
    // itself, which is structural (not in scope) — so leave the built selection untouched. (That's
    // "the field wasn't provided", not a compare-and-skip guard.)
    private static void RepointContoursIfChanged(
        ...
        if (contourDefs.Count == 0) {
            // Whole-sketch profile (no explicit contours): nothing to apply. Leave as built.
            return;
        }

        // Resolve the requested contour refs to live SketchRegion/SketchContour/SketchSegment via
        // the SAME path Add uses, scoped to the owning sketch.
        ...
        if (resolvedLive.Count == 0) {
            throw new InvalidOperationException(
                $"Revolve.Edit: no live contour matched the requested {contourDefs.Count} contour "
        ...
        SldworksLog.Information(
            "RevolveHandler.Edit: applied '{Name}' onto {Count} contour(s)",
    ...
    private static (double Angle, double? AngleB, bool MidPlane) ReadRevolveAngles(
        ...
        // SW quirk: *360Degrees variants store angle as the enum itself (GetRevolutionAngle returns 0).
        const double tau = 2.0 * Math.PI;
        switch (revolveType) {
            ...
            case (int)swRevolveType_e.swRevolveTypeTwoDirection360Degrees:
                // SW quirk: TwoDirection-360 doesn't expose A and B separately (sum to 2π).
                return (tau, null, false);
            ...
            default:
                throw new InvalidOperationException(
                    $"Inspect(Revolve): unknown swRevolveType_e value {revolveType}");
        }
    }

    private static JsonArray CaptureContours(IRevolveFeatureData2 data) {
        // SW quirk: data.Contours yields both SketchRegion (closed) and SketchContour (open).
        var arr = new JsonArray();
        var contours = data.Contours as object[];
        if (contours is null || contours.Length == 0) return arr;
        foreach (var contour in contours) {
            if (contour is null) {
                throw new InvalidOperationException("Inspect(Revolve): contour entry is null");
            }
            arr.Add(DefinitionCapture.Capture(contour)?.ToJson()
                ?? throw new InvalidOperationException(
                    $"Inspect(Revolve): contour entry of unsupported type {contour.GetType().Name} " +
    ...
    // FeatureRevolve2 silently sees zero bodies if they're on the wrong mark.
    ...
    private static void SelectFeatureScopeBodiesForAdd(File file, IReadOnlyList<Definition> bodies) {
        ...
        foreach (var def in bodies) {
            var live = DefinitionResolver.Resolve(file, def)
                ?? throw new InvalidOperationException(
                    "Revolve: feature_scope body did not resolve to a live entity");
            switch (live) {
                case Body2 b:    b.Select2(true, sd); break;
                case Feature ft: ft.Select2(true, MarkFeatureScopeBodies); break;
                default:
                    throw new InvalidOperationException(
                        $"Revolve: feature_scope entry resolved to unsupported type {live.GetType().Name}");
            }
        }
    }
    ...
    private static JsonNode CaptureAxis(File file, IRevolveFeatureData2 data) {
        ...
        object? axisObj = data.Axis
            ?? throw new InvalidOperationException(
                "Inspect(Revolve): axis is null (temporary axis not representable on the wire — promote the sketch line to a real entity)");
        // SW quirk: RefAxis returned for cylinder/cone temp-axis picks.
        return DefinitionCapture.Capture(axisObj)?.ToJson()
            ?? throw new InvalidOperationException(
                $"Inspect(Revolve): axis is unsupported runtime type {axisObj.GetType().Name}");
    }

    // ---- Selection helpers ---------------------------------------------------------

    // Appends contours to the existing selection at mark 0 — does NOT clear (caller
    // already cleared once at top of Add so the axis selection on mark 4 survives).
    private static void AppendContours(File file, IReadOnlyList<Definition> contours, string sketchName) {
        // SW quirk: contour selection at mark 0; SketchSegment uses Select4, SketchRegion/Contour use Select2.
        ...
        if (liveEntities.Count == 0) {
            throw new InvalidOperationException(
                $"Revolve: no live contour matched the captured " +
        ...
        foreach (var live in liveEntities) {
            switch (live) {
                ...
                default:
                    throw new InvalidOperationException(
                        $"Revolve: contour resolved to unsupported type {live.GetType().Name} " +
                        "(expected SketchRegion / SketchContour / SketchSegment)");
            }
        }
    }

    // Appends the sketch feature on mark 0 — does NOT clear (caller cleared once up front).
    private static void AppendSketch(File file, string sketchName) {
        if (string.IsNullOrEmpty(sketchName)) {
            throw new ArgumentException("Revolve: sketch name missing — sketch must be added first");
        }
        var feat = FindFeatureByName(file, sketchName)
            ?? throw new ArgumentException($"Sketch feature '{sketchName}' not found");
        // Mark 0 is the sketch profile slot. Append=true so the axis on mark 4 survives.
        feat.Select2(true, 0);
    }

    private static void SelectAxis(File file, Definition axisDef) {
        var live = DefinitionResolver.Resolve(file, axisDef)
            ?? throw new InvalidOperationException("Revolve axis definition did not resolve to a live entity");
        ...
        switch (live) {
            case Edge e:
                ...
                break;
            case Feature feat:
                // RefAxis / Origin / default-plane intersection select via the feature proxy.
            ...
            default:
                throw new InvalidOperationException(
                    $"Revolve axis resolved to unsupported type {live.GetType().Name}");
        }
    }
    ...
    private sealed record RevolveArgs(
        string SketchName,
        Definition Axis,
        double Angle,
        double? AngleB,
        ...
        internal static RevolveArgs Parse(JsonNode input) {
            var sketchName = input["sketch"]?["name"]?.GetValue<string>() ?? "";
            var axis = Definition.FromJson(input["axis"])
                ?? throw new ArgumentException("Revolve: 'axis' missing or malformed");
            ...
            if (angle <= 0 || angle > 2.0 * Math.PI + 1e-9) {
                throw new ArgumentException($"Revolve: angle {angle} radians outside (0, 2π]");
            }
            double? angleB = null;
            ...
            if (angleBNode is not null && angleBNode.GetValueKind() != System.Text.Json.JsonValueKind.Null) {
                ...
                if (v <= 0 || v > 2.0 * Math.PI + 1e-9) {
                    throw new ArgumentException($"Revolve: angle_b {v} radians outside (0, 2π]");
                }
                angleB = v;
            }
            var midPlane = ReadBool(input, "mid_plane", false);
            if (midPlane && angleB is not null) {
                throw new ArgumentException("Revolve: mid_plane and angle_b are mutually exclusive");
            }
            var reversed = ReadBool(input, "reversed", false);
            var merge = ReadBool(input, "merge", true);
            ...
            return new RevolveArgs(sketchName, axis, angle, angleB, midPlane, reversed, merge, contours, featureScope);
        }
    }
